...
const sendOTP = asyncHandler(async (req, res) => {
  ...
  const subject = "OTP CODE - syntridigital"
...
if(twofaAuthentication !== true && verifyEmailResendOtp !== true) {
 ...
 const subjectAdmin = "New User Registration - syntridigital"
 const send_to_Admin = process.env.EMAIL_USER
 const templateAdmin = NewUserEmailTemplate("Admin", user)
 const reply_toAdmin = "no_reply@syntridigital.com"
...
const introMessage = `This user ${user.firstname+" "+user.lastname} with email address ${user.email} is requesting an ID Verification`
  ...
  const subjectAdmin = "ID Verification Request - syntridigital"
  const send_to_Admin = process.env.EMAIL_USER
...
const adminApproveId = asyncHandler(async (req, res) => {
  ...
  const updatedUser = await user.save();
   ...
   // Send approval status email to the user
    ...
    const subject = "ID Approval Status - syntridigital"
    const send_to = user.email
 ...
 if (status === "VERIFIED") {
  ...
  const messages = [
    {
      to: user.email,
      from: "Support Team",
      subject: "Welcome to syntridigital",
      content: `Hello ${user.firstname+" "+user.lastname}, We're excited to have you on board. syntridigital is an international investment company that combines the infrastructure and abilities of an investor with a best-in-class team of operations professionals. This unique combination of skills  has allowed us to become a top international Investment Platform.For more enquiry kindly contact your account manager or write directly with our live chat support on our platform or you can send a direct mail to us at support@syntridigital.com.`,
    },
  ];
  ...
  await Mailbox.updateOne(
   ...
   const notificationObject = {
     ...
     message: `Your trade account has been activated successfully. Welcome to syntridigital`,
     route: "/dashboard"
   };
...
const adminSendCustomizedMail = asyncHandler(async (req, res) => {
  ...
  if (!errors.isEmpty()) {
   ...
   const reply_to = "no-reply@syntridigital.com"
...
const contactUs = asyncHandler(async (req, res) => {
  ...
  if (!firstname || !lastname || !email || !subject || !message ) {
    ...
    const subjectAdmin = "Contact Us - syntridigital"
    const send_to_Admin = process.env.EMAIL_USER
    const templateAdmin = adminGeneralEmailTemplate("Admin", introMessage)
    const reply_toAdmin = "no_reply@syntridigital.com"
...
const requestCard = asyncHandler(async (req, res) => {
  ...
  if (!firstname || !lastname || !email || !phone || !country || !cardType ) {
    ...
    const subjectAdmin = `${cardType} Request - syntridigital`
    const send_to_Admin = process.env.EMAIL_USER
    const templateAdmin = adminGeneralEmailTemplate("Admin", introMessage)
    const reply_toAdmin = "no_reply@syntridigital.com"
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    const resetPasswordLink = `https://syntridigital.com/auth/reset-password/${resetToken}`

    const subject = "Reset Password - syntridigital"
    const send_to = user.email
